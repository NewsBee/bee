<template>
  <div class="table">
    ...
    <div style="text-align: center">
      ...
      <button class="btn" @click="$router.push('GantiUser')">
        Ganti/Update data
      </button>
      <router-link
        class="btn"
    ...
    <table v-show="visible" class="table table-bordered">
      ...
      <tbody v-if="mahasiswa.length > 0">
        <tr v-for="row in mahasiswa" :key="row.id">
          ...
          <td>{{ row.password }}</td>
          <td v-if="msg == 'Ganti'">
            <router-link :to="'/userupd/' + row.id">Update</router-link>
          </td>
        </tr>
      </tbody>
...
export default {
  ...
  methods: {
    async getStudents() {
      await axios
        .get("https://62e8fab8249bb1284eb7bd0d.mockapi.io/idpass")
        .then((response) => {
